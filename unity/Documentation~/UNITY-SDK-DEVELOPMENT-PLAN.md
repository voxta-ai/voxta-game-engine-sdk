...
- [ ] Add a CI check that fails when regenerated output differs from checked-in output.
...
| 2026-09-22 | M5 CI | `.github/workflows/unity-package-ci.yml` | CI runs on Unity SDK pull requests and `main`, can be started manually, and checks daily for generated-protocol drift against the latest Voxta model. It creates a clean Unity project that installs the package through its public Git URL, imports Basic Chat Integration, runs its Unity tests, and builds Windows desktop Mono and IL2CPP players. Live-server and device-approval tests remain explicit and are not run. Hosted GitHub runners require the standard Unity activation secrets: `UNITY_LICENSE`, `UNITY_EMAIL`, and `UNITY_PASSWORD` for Personal, or `UNITY_SERIAL`, `UNITY_EMAIL`, and `UNITY_PASSWORD` for Pro; the drift check also requires a read-only `VOXTA_SERVER_READ_TOKEN` for the private server source. |
| | | | |
