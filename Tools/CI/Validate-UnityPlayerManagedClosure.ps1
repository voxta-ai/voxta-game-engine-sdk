...
if (-not (Test-Path -LiteralPath $dataDirectory -PathType Container)) {
    throw "Player data directory was not found: $dataDirectory"
}

$inventoryPath = Join-Path $RepositoryRoot 'unity/Runtime/Plugins/ThirdParty/RUNTIME-MODEL-DEPENDENCY-INVENTORY.json'
if (-not (Test-Path -LiteralPath $inventoryPath -PathType Leaf)) {
    throw "Runtime dependency inventory was not found: $inventoryPath"
}
...
$requiredAssemblies = @(
    ...
    'Voxta.Model.dll'
    $inventory.assembly | Where-Object { $_ -like 'Microsoft.AspNetCore.SignalR.*.dll' }
)
...
foreach ($assemblyName in $requiredAssemblies) {
    ...
    if ($matches.Count -ne 1) {
        throw "Expected exactly one $assemblyName in the player output, found $($matches.Count)."
    }

    $expected = @($inventory | Where-Object { $_.assembly -eq $assemblyName })
    if ($expected.Count -ne 1) {
        throw "Inventory does not uniquely identify $assemblyName."
    }

    $actualHash = (Get-FileHash -LiteralPath $matches[0].FullName -Algorithm SHA512).Hash
    if ($actualHash -ne $expected[0].assemblySha512) {
        throw "Player output hash mismatch for $assemblyName."
    }
}
