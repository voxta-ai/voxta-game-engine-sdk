...
| 2026-09-23 | M5 CI | `.github/workflows/unity-package-ci.yml`; `.github/workflows/voxta-model-update-compatibility-tests.yml`; `Tools/ModelDependencies/VoxtaModel/pinned-model.json` | Primary CI runs the pinned SDK on Unity SDK pull requests and pushed tags, and can be started manually. A separate daily canary checks NuGet for a newer Voxta.Model version and, when one exists, replaces only the model DLL in an isolated local package before running package tests and Windows Mono/Linux IL2CPP builds. It never changes the production pin. GameCI does not publish a Windows IL2CPP image for this Unity version. Live-server and device-approval tests remain explicit and are not run. Hosted GitHub runners require the standard Unity activation secrets: `UNITY_LICENSE`, `UNITY_EMAIL`, and `UNITY_PASSWORD` for Personal, or `UNITY_SERIAL`, `UNITY_EMAIL`, and `UNITY_PASSWORD` for Pro. No server checkout or server-read token is required. |
| | | | |
